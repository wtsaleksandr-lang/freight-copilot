...
function goToSettings(){close();const el=document.querySelector('[data-simple-tab="secrets"]')||document.querySelector('[data-tab="secrets"]');if(el)el.click();setTimeout(()=>{document.getElementById('ai-cfg-current')?.scrollIntoView({behavior:'smooth',block:'center'});},220);}
async function copyForOrchestrator(data){let d={};try{d=JSON.parse(decodeURIComponent(data||''));}catch(e){}const text=`LoadMode readiness issue:\n• ${d.name||'Issue'}: ${d.detail||''}${d.action?`\n• Suggested next step: ${d.action}`:''}`;const t=window.toast||function(){};try{await navigator.clipboard.writeText(text);t('Copied — paste it into your orchestrator chat.','success');}catch(e){t('Copy failed — select the text manually.','error');}}
function remButtons(item){const s=`${item.action||''} ${item.detail||''} ${item.summary||''} ${item.name||''}`.toLowerCase();const inApp=/(api key|ai key|\bkey\b|provider|vault|credential|password)/.test(s);const data=encodeURIComponent(JSON.stringify({name:item.name,detail:item.detail||item.summary||'',action:item.action||''}));return `<div class="rem-actions">${inApp?'<button type="button" class="rem-btn rem-fix">Fix in Settings</button>':''}<button type="button" class="rem-btn rem-copy" data-rem="${data}">Copy for orchestrator</button></div>`;}
function badge(state){return `<span class="feature-state feature-state-${esc(state)}">${esc(LABELS[state]||state)}</span>`;}
function card(feature){const actionable=feature.state==='unavailable'||feature.state==='setup_required';return `<article class="feature-readiness-row"><div class="feature-readiness-main"><strong>${esc(feature.name)}</strong><p>${esc(feature.summary)}</p>${feature.action?`<small>${esc(feature.action)}</small>`:''}${actionable?remButtons(feature):''}</div>${badge(feature.state)}</article>`;}
...
async function open(){
  ...
  backdrop.querySelector('.simple-dialog-close').addEventListener('click',finish);backdrop.querySelector('#system-check-close').addEventListener('click',finish);backdrop.addEventListener('click',(event)=>{const fx=event.target.closest('.rem-fix');const cp=event.target.closest('.rem-copy');if(fx){event.stopPropagation();goToSettings();return;}if(cp){event.stopPropagation();copyForOrchestrator(cp.dataset.rem);return;}if(event.target===backdrop)finish();});backdrop.addEventListener('keydown',(event)=>{if(event.key==='Escape'){event.preventDefault();finish();}});backdrop.querySelector('#system-check-again').addEventListener('click',run);backdrop.querySelector('.simple-dialog-close').focus();await run();
}
...
const PROV_STATE={configured:'Configured (environment)',missing:'Not set',stored_usable:'Stored securely',env_fallback:'Environment fallback',stored_locked:'Stored but locked'};
function priorityBlock(result){
  if(!Array.isArray(result.statusGroups))return '';
  const groups=result.statusGroups.filter((g)=>g.items&&g.items.length).map((g)=>{const m=GROUP_META[g.group]||{t:g.group};const actionable=g.group==='critical'||g.group==='setup';const items=g.items.map((it)=>`<li><strong>${esc(it.name)}</strong> — ${esc(it.detail)}${it.action?`<br><small>${esc(it.action)}</small>`:''}${actionable?remButtons(it):''}</li>`).join('');return `<section class="readiness-group readiness-group-${esc(g.group)}" data-group="${esc(g.group)}"><h4><span class="rg-dot" aria-hidden="true"></span>${esc(m.t)} <b>(${g.items.length})</b></h4><ul>${items}</ul></section>`;}).join('');
