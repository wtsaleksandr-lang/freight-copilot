...
function priorityBlock(result){
  ...
  const groups=result.statusGroups.filter((g)=>g.items&&g.items.length).map((g)=>{const m=GROUP_META[g.group]||{t:g.group};const items=g.items.map((it)=>`<li><strong>${esc(it.name)}</strong> — ${esc(it.detail)}${it.action?`<br><small>${esc(it.action)}</small>`:''}</li>`).join('');return `<section class="readiness-group readiness-group-${esc(g.group)}" data-group="${esc(g.group)}"><h4><span class="rg-dot" aria-hidden="true"></span>${esc(m.t)} <b>(${g.items.length})</b></h4><ul>${items}</ul></section>`;}).join('');
