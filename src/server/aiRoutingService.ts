...
export type ModelRole = {
  ...
  model: string;
  purpose: string;
  requiredCapabilities: Array<'text' | 'vision' | 'web' | 'tools' | 'reasoning'>;
...
const PRESETS: Record<Exclude<AiMode, 'custom'>, AiRoutingProfile> = {
  default: {
    mode: 'default',
    label: 'Everyday',
    summary: 'Fast, capable routing for daily shipment work and routine quotes.',
    recommendedFor: 'Document extraction, shipment updates, email drafting, rate calculations and routine web checks.',
    relativeCost: 'lowest',
    monthlyTargetUsd: 5,
    maxTaskCostUsd: 0.05,
    webPolicy: 'automatic',
  ...
  power: {
    mode: 'power',
    label: 'Power',
    summary: 'Stronger judgment and cross-checking for harder freight quotations.',
    recommendedFor: 'Complex rate comparisons, customs work, project planning and difficult customer replies.',
  ...
  ultimate: {
    mode: 'ultimate',
    label: 'Ultimate',
    summary: 'Parallel multi-model analysis with reconciliation and disagreement reporting.',
    recommendedFor: 'OOG, heavy haul, project cargo, claims and other high-value or high-risk decisions.',
    relativeCost: 'high',
    maxTaskCostUsd: 5,
    webPolicy: 'automatic',
    promptCaching: true,
...
export function listAiPresets(): AiRoutingProfile[] {
  return [clone(PRESETS.default), clone(PRESETS.power), clone(PRESETS.ultimate)];
}
...
export function validateProfile(input: unknown): AiRoutingProfile {
  ...
  const maxTaskCostUsd = Number(value.maxTaskCostUsd);
  if (!Number.isFinite(maxTaskCostUsd) || maxTaskCostUsd <= 0 || maxTaskCostUsd > 100) throw new Error('Maximum task cost must be between $0 and $100');
  return {
    mode: value.mode as AiMode,
    label: String(value.label || value.mode),
    summary: String(value.summary || ''),
    recommendedFor: String(value.recommendedFor || ''),
    relativeCost: (value.relativeCost || 'moderate') as AiRoutingProfile['relativeCost'],
    monthlyTargetUsd: value.monthlyTargetUsd == null ? undefined : Number(value.monthlyTargetUsd),
    maxTaskCostUsd,
    webPolicy: value.webPolicy as WebPolicy,
    promptCaching: value.promptCaching !== false,
    requireHumanApproval: Boolean(value.requireHumanApproval),
    roles: value.roles.map((role) => ({
      provider: role.provider,
      model: String(role.model || '').trim(),
      purpose: String(role.purpose || '').trim(),
      requiredCapabilities: Array.isArray(role.requiredCapabilities) ? role.requiredCapabilities : ['text'],
      parallelGroup: role.parallelGroup,
      fallback: Boolean(role.fallback),
    })).filter((role) => role.model && role.purpose),
  };
}

export async function getAiRoutingProfile(): Promise<AiRoutingProfile> {
  try {
    const db = createDbClient();
    const [row] = await db.select().from(appSettings).where(eq(appSettings.key, SETTING_KEY));
    if (row?.value) return validateProfile(JSON.parse(row.value));
  } catch (error) {
    console.warn('[ai-routing] could not read saved profile:', error);
  }
  return clone(PRESETS.default);
}

export async function saveAiRoutingProfile(input: unknown): Promise<AiRoutingProfile> {
  const profile = validateProfile(input);
  const db = createDbClient();
  const now = new Date();
  await db.insert(appSettings).values({ key: SETTING_KEY, value: JSON.stringify(profile), updatedAt: now })
    .onConflictDoUpdate({ target: appSettings.key, set: { value: JSON.stringify(profile), updatedAt: now } });
  return profile;
}

export function buildExecutionPlan(profile: AiRoutingProfile, task: { kind?: string; hasImages?: boolean; requiresFreshData?: boolean; highStakes?: boolean }) {
  const requiresWeb = profile.webPolicy === 'always' || (profile.webPolicy === 'automatic' && Boolean(task.requiresFreshData));
  const selected = profile.roles.filter((role) => {
    if (task.hasImages && role.requiredCapabilities.includes('vision')) return true;
    if (requiresWeb && role.requiredCapabilities.includes('web')) return true;
    return role.purpose.toLowerCase().includes('primary') || role.parallelGroup === 'analysis' || role.purpose.toLowerCase().includes('synthesis');
  });
  return {
    mode: profile.mode,
    task,
    webResearch: requiresWeb,
    promptCaching: profile.promptCaching,
    humanApprovalRequired: profile.requireHumanApproval || Boolean(task.highStakes),
    parallel: profile.mode === 'ultimate',
    maxTaskCostUsd: profile.maxTaskCostUsd,
    steps: selected.map((role, index) => ({ order: index + 1, ...role })),
    disagreementPolicy: profile.mode === 'ultimate' ? 'Show material disagreements and require user review before client-facing output.' : 'Use fallback only when validation fails or confidence is low.',
  };
}
