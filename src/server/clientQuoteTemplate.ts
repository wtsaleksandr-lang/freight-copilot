...
function customsDisclaimer(input: ClientQuoteInput): string {
  const isCustoms = input.template === 'import_usa' || input.template === 'import_canada' || input.template === 'export_clearance';
  if (!isCustoms) return '';
  return `<div class="terms disclaimer"><div><strong>Estimate only — not a formal customs quotation.</strong> Duties and taxes are calculated from published tariff schedules and may not include every applicable charge (e.g. Section 301 / Section 232 tariffs, antidumping / countervailing duties, excise, or other government-agency fees). The final duty, tax and import-clearance cost is determined only at formal cargo entry and evaluation by the customs authority.</div><div><strong>We are not a licensed customs broker</strong> and do not guarantee this estimate. A formal, reviewed quotation can be provided for an additional fee following a thorough evaluation of your shipping and commercial documentation.</div></div>`;
}
